...
import { Fullscreen, Check, Cancel, PaletteOutlined } from '@mui/icons-material';
    ...
    >
      ...
      {onApprove && onReject && hovered && approvalStatus === 'pending' && (
        <div style={{ position: 'absolute', bottom: 8, right: 8, zIndex: 7, display: 'flex', gap: 6 }}>
          <Tooltip title="Reject" slotProps={tooltipPopperProps}>
            <IconButton
              disabled={approvalDisabled}
              onClick={(e) => { e.stopPropagation(); onReject(); }}
              sx={{
                background: '#ffffff', padding: '5px', width: 36, height: 36, outline: '1px solid #be0e1c',
                boxShadow: '0px 1px 5px rgba(0,0,0,0.12), 0px 2px 2px rgba(0,0,0,0.14)',
                '&:hover': { background: '#fafafa' },
              }}
            >
              <Cancel style={{ fontSize: 18, color: '#be0e1c' }} />
            </IconButton>
          </Tooltip>
