...
export const ReviewPage = () => {
  ...
  const hasGeneratedSelected = hasSelection && selectedAssets.some(a => a.status === 'generated');

  const selectedHtmlMissingUrls = useMemo(() =>
    hasSelection && Array.from(selectedIds).some(id => {
      const asset = assets.find(a => a.id === id);
      if (!asset || asset.imageType !== 'HTML') return false;
      const ctas = getTemplateCtas(asset.templateId);
      const urls = destinationUrls[asset.id] ?? {};
      return ctas.some(cta => !urls[cta.key]);
    }),
  [selectedIds, assets, destinationUrls, hasSelection]);
  ...
  // Base filtered = search + filterState applied, but NOT the active tab status filter.
  // Used for both tab counts and filteredAssets so counts stay consistent with visible items.
  const baseFilteredAssets = useMemo(() => {
    const searched = nonApprovedAssets.filter((a) =>
      a.name.toLowerCase().includes(search.toLowerCase())
    );
    return applyAssetFilters(searched, filterState);
  }, [nonApprovedAssets, search, filterState]);
  ...
  const dynamicTabs = useMemo(() =>
    tabOrder
      ...
      .map((status) => ({
        ...
        count: baseFilteredAssets.filter((a) => a.status === status).length,
      })),
    [tabOrder, nonApprovedAssets, baseFilteredAssets]
  );
  ...
  const filteredAssets = useMemo(() =>
    baseFilteredAssets.filter((a) => activeTab === 'all' || a.status === activeTab),
  ...
  return (
    ...
    <div className="flex flex-col flex-1 min-h-0 overflow-hidden" style={{ background: '#ffffff', margin: 8, borderRadius: 8, boxShadow: '0 1px 4px rgba(0,0,0,0.08)' }}>
        ...
        {hasSelection ? (
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, flex: 1 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, backgroundColor: '#F4F5F6', padding: '4px 12px', borderRadius: 100, width: 'fit-content' }}>
              ...
              {approvalEnabled ? (
                /* Review mode: only the status selector */
                <Tooltip title={changeStatusDisabled ? changeStatusTooltip : ''} placement="bottom">
                  <span style={{ flexShrink: 0, width: 180, display: 'inline-flex' }}>
                    <FormControl size="small" sx={{ width: 180 }} disabled={changeStatusDisabled}>
                      <Select
                        value=""
                        displayEmpty
                        renderValue={() => {
                ...
                /* Assets mode: additional action buttons */
                <>
                  {/* Add Destination URLs — shown when HTML assets are selected; disabled if non-HTML also selected */}
                  {selectedHtmlAssets.length > 0 && (
                    <Tooltip
                      title={selectedHasNonHtml ? 'Select only HTML assets to configure destination URLs.' : ''}
                      placement="bottom"
                    >
                      <span style={{ display: 'inline-flex', flexShrink: 0 }}>
                        <button
      ...
      {nonApprovedAssets.length > 0 && <div style={{ margin: '0px 16px',flexShrink: 0, background: '#ffffff', borderBottom: '1px solid #e0e0e0', display: 'flex' }}>
        {([...dynamicTabs, { status: 'all' as const, label: 'All', count: baseFilteredAssets.length }]).map(({ status, label, count }) => {
